...
/* ── 스크랩북 카드 ── */
.scrap-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px; margin-top: 12px; }
.scrap-card {
    background: #fff;
    border: 1px solid #e2e8f0;
    border-radius: 14px;
...
.scrap-card:hover { box-shadow: 0 6px 24px rgba(0,0,0,0.12); transform: translateY(-2px); }
.scrap-card-thumb { width: 100%; height: 160px; object-fit: cover; background: #f1f5f9; }
.scrap-card-thumb-placeholder {
    width: 100%; height: 120px;
    display: flex; align-items: center; justify-content: center;
    font-size: 2.5rem; background: #f8fafc;
}
.scrap-card-body { padding: 14px 16px 10px; flex: 1; }
.scrap-card-type {
...
.type-url   { background: #dcfce7; color: #15803d; }
.type-memo  { background: #fef3c7; color: #b45309; }
.scrap-card-title { font-size: 0.95rem; font-weight: 700; color: #1e293b; margin: 0 0 6px; line-height: 1.4; }
.scrap-card-meta  { font-size: 0.75rem; color: #94a3b8; margin-bottom: 6px; }
.scrap-card-summary { font-size: 0.82rem; color: #475569; line-height: 1.6; margin-bottom: 8px; }
.scrap-tag {
    display: inline-block; background: #f1f5f9; color: #64748b;
    font-size: 0.7rem; border-radius: 20px; padding: 2px 9px; margin: 2px 2px 2px 0;
}
.scrap-card-footer { padding: 8px 16px 12px; border-top: 1px solid #f1f5f9; }
</style>
...
# ── 스크랩북 저장/불러오기 ──────────────────────────────────
def get_scraps_file(session_id):
    return os.path.join(HISTORY_DIR, f"scraps_{session_id}.json")

def load_scraps(session_id):
    path = get_scraps_file(session_id)
    if os.path.exists(path):
        try:
            with open(path, "r", encoding="utf-8") as f:
                return json.load(f)
...
def save_scraps(session_id, scraps):
    try:
        with open(get_scraps_file(session_id), "w", encoding="utf-8") as f:
            json.dump(scraps, f, ensure_ascii=False, indent=2)
    except Exception:
        pass

def delete_scrap(session_id, scrap_id):
    scraps = load_scraps(session_id)
    scraps = [s for s in scraps if s.get("id") != scrap_id]
    save_scraps(session_id, scraps)

def fetch_url_meta(url):
    """URL에서 제목, 설명, 썸네일 추출"""
    meta = {"title": url, "description": "", "image": ""}
    try:
        # 유튜브 처리
        yt_match = re.search(r"(?:youtube\.com/watch\?v=|youtu\.be/)([A-Za-z0-9_\-]{11})", url)
        if yt_match:
            vid = yt_match.group(1)
            meta["image"] = f"https://img.youtube.com/vi/{vid}/hqdefault.jpg"
            meta["is_youtube"] = True
            meta["video_id"] = vid
        # OG 태그 파싱
        ...
        if og_title:
            ...
            meta["title"] = title_tag.group(1)
        if og_desc:
            meta["description"] = og_desc.group(1)[:200]
        if og_img and not meta["image"]:
            meta["image"] = og_img.group(1)
    except Exception:
        pass
    return meta
...
# ── 페이지 4: 주식 정보 ──────────────────────────────────────
    ...
    st.markdown("""
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:6px 0 4px 0">
  <div style="font-size:0.75rem;color:#94a3b8;padding:4px 2px">한국 대형 우량주 대표 지수</div>
  <div style="font-size:0.75rem;color:#94a3b8;padding:4px 2px">한국 중소·벤처기업 위주 지수</div>
  <div style="font-size:0.75rem;color:#94a3b8;padding:4px 2px">미국 500대 대형주 대표 지수</div>
...
# ══════════════════════════════════════════════════════════════
    ...
    with st.expander("➕ 새 스크랩 추가", expanded=len(all_scraps) == 0):
        ...
        if scrap_type == "📸 이미지":
            uploaded = st.file_uploader("이미지 파일 업로드", type=["png", "jpg", "jpeg", "gif", "webp"], key="scrap_img_uploader")
            if uploaded:
                img_bytes = uploaded.read()
                scrap_image_b64 = base64.b64encode(img_bytes).decode()
                ext = uploaded.name.split(".")[-1].lower()
                mime = "image/gif" if ext == "gif" else "image/webp" if ext == "webp" else f"image/{ext}"
                scrap_thumb = f"data:{mime};base64,{scrap_image_b64}"
                st.image(uploaded, caption="미리보기", use_container_width=True)

        elif scrap_type == "🔗 링크":
            url_input = st.text_input("URL 입력", placeholder="https://...", key="scrap_url_input")
            if url_input and url_input.startswith("http"):
                scrap_content = url_input
                with st.spinner("링크 정보 불러오는 중..."):
                    url_meta = fetch_url_meta(url_input)
                if url_meta.get("image"):
                    st.image(url_meta["image"], width=200)
                if url_meta.get("title") and url_meta["title"] != url_input:
                    st.caption(f"📄 {url_meta['title']}")
        ...
        if save_btn:
            ...
            if not content_ok:
                ...
                if not auto_title:
                    if scrap_type == "📸 이미지":
                        auto_title = f"이미지 스크랩 {datetime.now().strftime('%m/%d %H:%M')}"
                    elif scrap_type == "🔗 링크":
                        auto_title = url_meta.get("title", scrap_content)[:60] or scrap_content[:60]
                    else:
                ...
                if want_ai:
                    with st.spinner("AI가 요약 중..."):
                        if scrap_type == "🔗 링크":
                            summary_prompt = f"다음 링크 정보를 주식·경제 공부 관점에서 2~3줄로 요약해줘.\n제목: {url_meta.get('title','')}\n설명: {url_meta.get('description','')}\nURL: {scrap_content}"
                        else:
...
# ══════════════════════════════════════════════════════════════
    ...
    st.caption("거래 내역을 직접 입력하고 수익률을 확인해보세요.")
    ...
    with st.expander("⚙️ 시드머니 설정 & 초기화", expanded=False):
        ...
        with ec1:
            seed_input = st.number_input(
                "시드머니 (원)", min_value=100_000, max_value=1_000_000_000,
                value=int(sim.get("init_cash", SIM_INIT_CASH)),
                step=1_000_000, format="%d", key="sim_seed_input",
            )
            st.caption(f"설정 금액: **{seed_input:,}원**")
        with ec2:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("🔄 초기화 실행", type="primary", use_container_width=True, key="sim_reset_btn"):
                new_sim = {"cash": seed_input, "holdings": {}, "trades": [], "init_cash": seed_input}
                st.session_state.sim = new_sim
                save_simulation(session_id, new_sim)
                st.success(f"✅ {seed_input:,}원으로 초기화!")
                st.rerun()

    sim = st.session_state.sim
    st.markdown("---")

    tab_entry, tab_portfolio, tab_history = st.tabs(["📝 거래 입력", "💼 포트폴리오", "📋 거래 내역"])

    # ════════════════════════════════════════════════════════════
    # 탭 1: 거래 입력 폼
    # ════════════════════════════════════════════════════════════
    with tab_entry:
        st.markdown(f"**💰 현재 잔여 현금: {sim['cash']:,.0f}원**")
        st.markdown("---")
        st.markdown("#### ➕ 거래 추가")

        f1, f2 = st.columns(2)
        with f1:
            t_name   = st.text_input("종목명", placeholder="예: 삼성전자 / Tesla", key="t_name")
            t_type   = st.radio("구분", ["🟢 매수", "🔴 매도"], horizontal=True, key="t_type")
            t_shares = st.number_input("수량 (주)", min_value=1, value=1, step=1, key="t_shares")
        with f2:
            t_price  = st.number_input("단가 (원)", min_value=1, value=50000, step=100, key="t_price",
                                       help="미국 주식은 원화 환산 금액으로 입력하세요")
            t_date   = st.date_input("거래일", value=datetime.today(), key="t_date")
        ...
        if st.button("✅ 거래 추가", type="primary", use_container_width=True,
                     key="sim_add_trade", disabled=not can_add):
            trade_key = t_name.strip()
            if is_buy:
                sim["cash"] -= total_amt
            ...
            sim["trades"].insert(0, {
                ...
                "price":     t_price,
                "total_krw": total_amt,
                "pnl_krw":   pnl_krw if not is_buy else 0,
                "memo":      t_memo,
            })
    ...
    with tab_portfolio:
        ...
        st.caption("※ 현재가는 평균단가 기준 — 실제 시세와 다를 수 있습니다.")

        holdings_val = sum(h["avg_price"] * h["shares"] for h in sim["holdings"].values())
        total_assets = sim["cash"] + holdings_val
        total_pnl    = total_assets - sim["init_cash"]
        total_pct    = total_pnl / sim["init_cash"] * 100 if sim["init_cash"] else 0

        p1, p2, p3, p4 = st.columns(4)
        with p1: st.metric("💰 잔여 현금",  f"{sim['cash']:,.0f}원")
        with p2: st.metric("📈 보유 평가액", f"{holdings_val:,.0f}원")
        with p3: st.metric("🏦 총 자산",    f"{total_assets:,.0f}원")
        with p4:
            em = "🟢" if total_pnl >= 0 else "🔴"
            st.metric(f"{em} 총 손익", f"{total_pnl:+,.0f}원", delta=f"{total_pct:+.2f}%")

        # 도넛 차트
        if sim["holdings"] or sim["cash"] > 0:
            pie_labels = ["현금"] + [h["name"] for h in sim["holdings"].values()]
            pie_values = [sim["cash"]] + [h["avg_price"] * h["shares"] for h in sim["holdings"].values()]
            ...
            pie_fig.update_layout(
                title="자산 구성", paper_bgcolor="#0e1117", font_color="white",
    ...
    with tab_history:
        ...
        if not sim["trades"]:
            st.info("아직 거래 내역이 없어요. 거래 입력 탭에서 첫 거래를 시작해보세요! 🚀")
        else:
            ...
            if sell_trades:
                ...
                line_fig.add_trace(go.Scatter(
                    x=[t["date"] for t in reversed(sell_trades)], y=cum,
                    mode="lines+markers",
                ...
                line_fig.update_layout(
                    ...
                    font_color="white", height=260, margin=dict(t=10, b=20),
                )
